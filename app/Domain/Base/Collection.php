...
use app\Domain\Base\Exceptions\LogicException;
use app\Domain\Base\Exceptions\UnexpectedValueException;
use app\Domain\Base\Interfaces\MetaDataInterface;
use Traversable;
...
class Collection implements CollectionInterface
{
    ...
    protected function checkType($object)
    {
        if (!$object instanceof $this->_type) {
            throw new UnexpectedValueException('Неверный тип объекта');
        }
    }
    ...
    {
        throw new LogicException('Коллекцию нельзя менять');
    }
    ...
    public function sortByField(string $fieldName, string $direction = 'asc'): CollectionInterface
    {
        ...
        if ($direction != 'asc' or $direction != 'desc') {
            throw new UnexpectedValueException('Неизвестное направление сортировки');
        }
